...
void reorder(int pos, std::vector<int> selected) {
    node* n = mHeader->next;
    for (int i = 0; i < pos; i++) {
        n = n->next;  // หาโหนดที่ตำแหน่ง pos
    }

    for (int sel : selected) {
        node* t = mHeader->next;
        for (int i = 0; i < sel; i++) {
            t = t->next;  // หาโหนดที่ตำแหน่ง sel
        }

        // ตัดโหนด t ออกจากตำแหน่งเดิม
        t->prev->next = t->next;
        t->next->prev = t->prev;

        // แทรก t ก่อนตำแหน่ง n
        t->next = n;
        t->prev = n->prev;
        ...
        // ปรับตำแหน่ง n ให้ชี้ไปยังโหนดถัดไปของโหนดที่เพิ่งแทรก
        n = t->next;
    }
}
