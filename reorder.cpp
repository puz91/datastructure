...
void reorder(int pos, std::vector<int> selected) {
    // ตั้ง pointer ที่ตำแหน่ง pos
    node* insertPos = mHeader->next;
    for (int i = 0; i < pos; ++i) {
        insertPos = insertPos->next;
    }

    // จัดการ selected ตามลำดับ
    std::vector<node*> nodesToMove;
    for (int sel : selected) {
        node* target = mHeader->next;
        for (int i = 0; i < sel; ++i) {
            target = target->next;
        }
        // เก็บ node ที่ต้องการย้ายลง vector
        nodesToMove.push_back(target);
    }
    // ทำการย้าย node ที่เลือกไปตำแหน่ง insertPos
    for (auto target : nodesToMove) {
        // ตัด target ออกจากตำแหน่งเดิม
        target->prev->next = target->next;
        target->next->prev = target->prev;

        // แทรก target ก่อนตำแหน่ง insertPos
        target->next = insertPos;
        target->prev = insertPos->prev;

        insertPos->prev->next = target;
        insertPos->prev = target;
    }
}
